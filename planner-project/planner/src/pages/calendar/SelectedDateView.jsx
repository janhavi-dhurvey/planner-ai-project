...
const SelectedDateView = ({ goals, loading, error, selectedDate }) => {
  ...
  const getBarWidth = (duration) => {
    ...
    // Cap width to prevent horizontal overflow
    return `${Math.min(base + d * 2, 600)}px`; 
  };
  ...
  /* =========================================
      SAFE DATE FORMATTER (CRASH PREVENTER)
  ========================================= */
  const safeDateDisplay = (dateStr, options = {}) => {
    try {
  ...
  // Ensure goals is always an array to prevent .length crash
    ...
    return (
      ...
      >
        ...
        <p style={{ opacity: 0.7, maxWidth: "400px", margin: "auto", lineHeight: "1.6" }}>
          You don't have any goals scheduled for <strong>{safeDateDisplay(selectedDate)}</strong>.
          <br />
  ...
  return (
    <div className="timeline-container" style={{ marginTop: "30px", animation: "fadeIn 0.5s ease" }}>
      <h3 style={{ marginBottom: "25px", opacity: 0.8, fontSize: "1.2rem" }}>
        Timeline for {safeDateDisplay(selectedDate, { month: 'long', day: 'numeric' })}
      </h3>
